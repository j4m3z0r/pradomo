...
class AndroidMowerTransport(
    ...
    private val callback = object : BluetoothGattCallback() {
        override fun onConnectionStateChange(g: BluetoothGatt, status: Int, newState: Int) {
            when (newState) {
                // Request a large MTU first: the joystick payload is 24 base64 bytes,
                // which doesn't fit in a single write at the default MTU of 23 (max 20
                // bytes) — the angular/turn float would be truncated away. Telemetry
                // notifications also need the larger MTU to arrive un-fragmented.
                BluetoothProfile.STATE_CONNECTED -> if (!g.requestMtu(517)) g.discoverServices()
        ...
        override fun onMtuChanged(g: BluetoothGatt, mtu: Int, status: Int) {
            // Proceed once MTU negotiation finishes (success or not — discovery still works).
            g.discoverServices()
        }
